...
export async function POST(request: NextRequest) {
  try {
    ...
    const { entries, weekStart, currentWeight, goalWeight, startWeight, targetDate, height, sex, activityLevel } = body;
...
- Days to Target: ${Math.ceil((new Date(targetDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24))}
  ...
  "recommendedSteps": number
    ...
    try {
      ...
      aiResponse = {
        ...
        recommendedCalories: Math.round(2000 - (currentWeight - goalWeight) * 50),
        recommendedSteps: Math.min(12000, 8000 + Math.round((currentWeight - goalWeight) * 200))
      };
    }
